$(function() {
    ...
    $('#gl').on('change', function() {
        $.when(getStudents($(this).val()).then(
                function(response) {
                    response.forEach(element => {
                        element['action'] = '<div"><a href="'+ BASE_URI + '/update/edit/student/' + element.id + '"><button class="button" style="width:5.5rem; border-radius:0px;">Edit</button></a><button data-id="' + element.id + '" data-role="' + element.role + '" class="button" style="width:5.5rem; margin-left:2%; border-radius:0px;" id="deleteStudent">Delete</button></div>';
                    });
    ...
    $.when(getAdmins().then(
        function(response) {
            response.forEach(element => {
                element['action'] = '<div"><a href="'+ BASE_URI + '/update/edit/admin/' + element.id + '"><button class="button" style="width:5.5rem; border-radius:0px;">Edit</button></a><button data-id="' + element.id + '" data-role="' + element.role + '" class="button" style="width:5.5rem; margin-left:2%; border-radius:0px;" id="deleteStudent">Delete</button></div>';
            });
